...
source "virtualbox-iso" "gen"{
  ...
  vboxmanage           = [["modifyvm", "{{ .Name }}", "--memory", "${var.memsize}"], ["modifyvm", "{{ .Name }}", "--cpus", "${var.numvcpus}"], ["modifyvm", "{{ .Name }}", "--firmware", "EFI"], ["storageattach", "{{ .Name }}", "--storagectl", "SATA Controller", "--type", "dvddrive", "--port", "3", "--medium", "./scripts/uefi/gui/autounattend.iso"]]
  vm_name              = "${var.vm_name}"
  winrm_insecure       = true
...
build {
  sources = ["source.virtualbox-iso.gen", "source.vmware-iso.gen"]

  provisioner "powershell" {
    only         = ["vmware-iso", "!var.fast"]
    pause_before = "1m0s"
    scripts      = ["scripts/vmware-tools.ps1"]
  }

  provisioner "powershell" {
    only         = ["virtualbox-iso", "!var.fast"]
    pause_before = "1m0s"
